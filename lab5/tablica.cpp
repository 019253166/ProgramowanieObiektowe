...
void Tablica::zmien_rozmiar(int Nowe_wier, int Nowe_kol){
	...
	for(int x=0; x<Nowe_wier;x++){
		for(int y=0; y<Nowe_kol;y++){
			tab[x][y].zwroc_liczba() = 0;
		}
	}
	if(wiersze>Nowe_wier || kolumny>Nowe_kol){
		for(int x=0; x<Nowe_wier; x++){
			for(int y=0; y<Nowe_kol;y++){
				tab[x][y].zwroc_liczba() = tablica[x][y].zwroc_liczba();
			}
		}
	}
	else{
...
int Tablica::zmien_zawartosc_komorki(int w_komorka, int k_komorka, int zmiana){
	...
	tablica[w_komorka-1][k_komorka-1].zwroc_liczba()=zmiana;
	return 0;
}
...
int Tablica::zmien_zawartosc_komorki_tekst(int w_komorka, int k_komorka, string zmiana){
	...
	tablica[w_komorka-1][k_komorka-1].zwroc_tekst()=zmiana;
	return 0;
}
...
int Tablica::Tab_z_pliku_tekst(){
	...
	if(z_pliku.good()==true){
		...
		for(int x=0; x<wiersze; x++) {
			for(int y=0; y<kolumny; y++){
				z_pliku>>tablica[x][y].zwroc_tekst();
		}
			}
...
int Tablica::Tab_do_pliku(){
	...
	if(do_pliku.good()==true){
		...
		do_pliku<<kolumny<<endl;
			for(int x=0; x<wiersze; x++) {
				for(int y=0; y<kolumny; y++){
					do_pliku<<tablica[x][y].zwroc_liczba()<<"\t";
				}
				do_pliku<<endl;
			}
		do_pliku.close();
...
int Tablica::Suma_W(int nr_w){
	...
	suma_w+=tablica[nr_w-1][i].zwroc_liczba();
	}
...
int Tablica::Suma_K(int nr_k){
	...
	int suma_k=0;
	for(int i=0;i<wiersze;i++){
		suma_k+=tablica[i][nr_k-1].zwroc_liczba();
	}
	cout<<suma_k<<endl;
	return 0;
}
...
int Tablica::Min_K(int nr_k){
	...
	int min_k=tablica[0][nr_k-1].zwroc_liczba();
	for(int i=0;i<wiersze;i++){
		if(min_k>tablica[i][nr_k-1].zwroc_liczba()){
			min_k=tablica[i][nr_k-1].zwroc_liczba();
		}
	}
	cout<<min_k<<endl;
	return 0;
...
int Tablica::Min_W(int nr_w){
	...
	for(int i=0;i<kolumny;i++){
		if(min_w>tablica[nr_w-1][i].zwroc_liczba()){
			min_w=tablica[nr_w-1][i].zwroc_liczba();
		}
	}
	cout<<min_w<<endl;
	return 0;
...
int Tablica::Max_W(int nr_w){
	...
	for(int i=0;i<kolumny;i++){
		if(max_w<tablica[nr_w-1][i].zwroc_liczba()){
			max_w=tablica[nr_w-1][i].zwroc_liczba();
		}
	}
	cout<<max_w<<endl;
	return 0;
